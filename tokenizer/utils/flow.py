...
class DiTCFM(ConditionalCFM):
    ...
    def compute_loss(
        ...
        attn_mask = create_attention_mask(mask.astype(bool), causal=self.use_causal_mask) if mask is not None else None
    ...
    def solve_euler(
        ...
        for step in range(1, len(t_span)):
            ...
            if self.inference_cfg_rate > 0:
                ...
                v_cond = self.estimator(x, t_expanded, ref=mu, spk_emb=spks, mask=attn_mask)
                ...
                # Apply CFG
                v = (1.0 + self.inference_cfg_rate) * v_cond - self.inference_cfg_rate * v_uncond
            else:
                # Standard prediction
