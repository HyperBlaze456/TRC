...
def visualize_masks():
    ...
    for i in range(len(padded_audio)):
        ax.plot(
            padded_audio[i], label=f"Audio {i + 1} (len={audio_lengths[i]})", alpha=0.7
        )
    ...
    non_causal_sample = (
        jnp.ones((max_length, max_length)) * mask_2d[sample_idx, None, :]
    )
    causal_sample = causal_mask[sample_idx, 0, :, :]

    # Stack them side by side
    comparison = jnp.hstack(
        [non_causal_sample, jnp.ones((max_length, 5)) * 0.5, causal_sample]
    )
