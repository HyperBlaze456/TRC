...
class VectorQuantizer(nnx.Module):
    ...
    def get_distances(self, z):
        ...
        Args:
            z: Input embeddings [B, T, D]

        Returns:
            L2 distances [B, T, codebook_size]
        """
...
class ResidualVQ(nnx.Module):
    def __init__(
        self, codebook_size: int, dim: int, rngs: nnx.Rngs, num_codebooks: int = 1
    ):
        self.dim = dim
...
class PhonemeBSQQuantizer(nnx.Module):
    ...
    def __init__(
        ...
        self.phoneme_vq = VectorQuantizer(
            codebook_size=phoneme_codebook_size, dim=input_dim, rngs=rngs
        )
    ...
    def __call__(
        ...
        bsq_quantized, acoustic_codes = self.acoustic_bsq(vq_residual)

        # Combine both quantizations
        reconstructed = phoneme_quantized + bsq_quantized
    ...
    def get_phoneme_logits(self, x: jax.Array, temperature: float = 1.0):
        """Get phoneme logits for CTC loss.

        Converts L2 distances to logits using negative distance and softmax.
