...
class DiT(nnx.Module):
    def __init__(
        ...
        if in_channels is not None:
            ...
            self.x_embedder = nnx.Linear(
                in_channels * patch_size * 2, hidden_size, rngs=rngs
            )
        else:
        ...
        self.final_layer = FinalLayer(
            hidden_size, patch_size, out_channels or in_channels, rngs, learn_sigma
        )
    ...
    def __call__(self, x, t, ref=None, spk_emb=None, mask=None):
        ...
        if spk_emb is not None:
            ...
            spk_emb = jnp.broadcast_to(
                spk_emb, (B, N, self.hidden_size)
            )  # [B, N, hidden_size]
        else:
...
class FinalLayer(nnx.Module):
    def __init__(
        self, hidden_size, patch_size, out_channels, rngs: nnx.Rngs, learn_sigma=True
    ):
